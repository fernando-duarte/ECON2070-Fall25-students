...
c_f <- model$paths$c_on_saddle(k0)
c_a <- min(ch * 1.30, 0.95 * (limits_for_model(model)$ymax))
c_b <- ch
frac <- model$plot_opts$c_frac_FB
if (!is.finite(frac)) {
  frac <- 0.5
}
c_c <- if (is.finite(c_f) && is.finite(c_b)) (1 - frac) * c_f + frac * c_b else 0.92 * ch
c_d <- max(0.12 * ch, 0.02 * (limits_for_model(model)$ymax))
message(sprintf("k* = %.4g, c* = %.4g, k0 = %.4g", ss$k, ss$c, k0))
message(sprintf(
  "c0 on saddle (F) = %.4g | A=%.4g, B=%.4g, C=%.4g, D=%.4g",
  c_f,
  c_a,
  c_b,
  c_c,
...
for (i in seq_along(src_figs)) {
  if (file.exists(dest_figs[i])) {
    file.remove(dest_figs[i])
  }
...
if (TRUE) {
  # Enabled to regenerate halfk variants with tight margins
  base_fig4_ylim <- model$plot_opts$fig4_ylim_frac
  if (!is.finite(base_fig4_ylim) || base_fig4_ylim <= 0) {
    base_fig4_ylim <- 0.5
  }
  plot_overrides_4_long <- merge_options(
    plot_overrides,
    list(
  ...
  if (file.exists(dest4_long)) {
    file.remove(dest4_long)
  }
  if (file.exists(src4_long)) {
    invisible(file.rename(src4_long, dest4_long))
  }
  ...
  plot_overrides_4_vlong <- merge_options(
    plot_overrides,
    list(
  ...
  tmp4_vlong <- setdiff(
    Sys.glob(file.path(output_dir, sprintf("neoclassical_growth_very_long-*.%s", img_ext))),
    src4_vlong
  )
  if (length(tmp4_vlong)) {
    unlink(tmp4_vlong)
  }
  if (file.exists(dest4_vlong)) {
    file.remove(dest4_vlong)
  }
  if (file.exists(src4_vlong)) {
    invisible(file.rename(src4_vlong, dest4_vlong))
  }
  ...
  if (!is.finite(k_gap_share) || k_gap_share <= 0 || k_gap_share >= 1) {
    k_gap_share <- 0.4
  }
  ...
  plot_overrides_6 <- merge_options(
    plot_overrides,
    list(
  ...
  if (file.exists(dest6)) {
    file.remove(dest6)
  }
  if (file.exists(src6)) {
    invisible(file.rename(src6, dest6))
  }
  ...
  plot_overrides_7 <- merge_options(
    plot_overrides,
    list(
  ...
  if (file.exists(dest7)) {
    file.remove(dest7)
  }
  if (file.exists(src7)) {
    invisible(file.rename(src7, dest7))
  }
  ...
  if (file.exists(src4)) {
    ...
    tmp4b <- setdiff(
      Sys.glob(file.path(output_dir, sprintf("neoclassical_growth_halfk-*.%s", img_ext))),
      src4
    )
    if (length(tmp4b)) {
      unlink(tmp4b)
    }
    if (file.exists(dest4)) {
      file.remove(dest4)
    }
  ...
  tmp4b_long <- setdiff(
    Sys.glob(file.path(output_dir, sprintf("neoclassical_growth_halfk_long-*.%s", img_ext))),
    src4b_long
  )
  if (length(tmp4b_long)) {
    unlink(tmp4b_long)
  }
  if (file.exists(dest4b_long)) {
    file.remove(dest4b_long)
  }
  if (file.exists(src4b_long)) {
    invisible(file.rename(src4b_long, dest4b_long))
  }
  ...
  obsolete_halfk <- file.path(
    output_dir,
    sprintf(c("ramsey_fig4b_paths.%s", "ramsey_fig4b_paths_long.%s"), img_ext)
  )
