...
pdf_path <- file.path(output_dir, "neoclassical_phase_suite.pdf")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Helper to render canonical suite into multi-page pdf
...
grDevices::pdf(pdf_path, width = dev_width, height = dev_height)
...
par(family = base_family)
render_phase_suite(model, which = "combined")

message("Phase diagrams written to ", pdf_path)
...
# ---- Derived Outputs ----
# Also generate SVGs (one file per page) alongside the PDF
img_ext <- "svg"
png_pattern <- sub("\\.pdf$", paste0("-%d.", img_ext), pdf_path)
unlink(Sys.glob(sub("%d", "*", png_pattern, fixed = TRUE)))
ramsey_save(model, file = png_pattern, which = "combined", device = img_ext, width = dev_width, height = dev_height)

fig_keys <- c("cdot_locus", "kdot_locus", "phase_quadrants", "paths_baseline", "saddle_full")
src_figs <- file.path(output_dir, sprintf("ramsey_phase_suite-%d.%s", seq_along(fig_keys), img_ext))
dest_figs <- file.path(output_dir, paste0("fig_ramsey_", fig_keys, ".", img_ext))
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_baseline_color.%s", img_ext))
)
}
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_long_color.%s", img_ext))
)
...
out4_vlong_pattern <- file.path(output_dir, sprintf("ramsey_paths_very_long-%%d.%s", img_ext))
unlink(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_very_long-*.%s", img_ext))))
ramsey_save(model4_vlong, file = out4_vlong_pattern, which = "combined", device = img_ext, width = dev_width, height = dev_height)
src4_vlong <- file.path(output_dir, sprintf("ramsey_paths_very_long-4.%s", img_ext))
dest4_vlong <- file.path(output_dir, sprintf("fig_ramsey_paths_very_long.%s", img_ext))
tmp4_vlong <- setdiff(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_very_long-*.%s", img_ext))), src4_vlong)
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_very_long_color.%s", img_ext))
)
...
out6_pattern <- file.path(output_dir, sprintf("ramsey_paths_near_ss-%%d.%s", img_ext))
unlink(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_near_ss-*.%s", img_ext))))
ramsey_save(model6, file = out6_pattern, which = "combined", device = img_ext, width = dev_width, height = dev_height)
src6 <- file.path(output_dir, sprintf("ramsey_paths_near_ss-4.%s", img_ext))
dest6 <- file.path(output_dir, sprintf("fig_ramsey_paths_near_ss.%s", img_ext))
tmp6 <- setdiff(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_near_ss-*.%s", img_ext))), src6)
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_near_ss_color.%s", img_ext))
)
...
out7_pattern <- file.path(output_dir, sprintf("ramsey_paths_halfgap-%%d.%s", img_ext))
unlink(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfgap-*.%s", img_ext))))
ramsey_save(model7, file = out7_pattern, which = "combined", device = img_ext, width = dev_width, height = dev_height)
src7 <- file.path(output_dir, sprintf("ramsey_paths_halfgap-4.%s", img_ext))
dest7 <- file.path(output_dir, sprintf("fig_ramsey_paths_halfgap.%s", img_ext))
tmp7 <- setdiff(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfgap-*.%s", img_ext))), src7)
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_halfgap_color.%s", img_ext))
)
...
out4b_pattern <- file.path(output_dir, sprintf("ramsey_paths_halfk-%%d.%s", img_ext))
# Clean previous 4b files
unlink(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfk-*.%s", img_ext))))
...
src4 <- file.path(output_dir, sprintf("ramsey_paths_halfk-4.%s", img_ext))
dest4 <- file.path(output_dir, sprintf("fig_ramsey_paths_halfk.%s", img_ext))
if (file.exists(src4)) {
  # Remove any other temporary 4b pages, leaving only the -4 file
  tmp4b <- setdiff(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfk-*.%s", img_ext))), src4)
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_halfk_color.%s", img_ext))
)
...
out4b_long_pattern <- file.path(output_dir, sprintf("ramsey_paths_halfk_long-%%d.%s", img_ext))
unlink(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfk_long-*.%s", img_ext))))
ramsey_save(model4b_long, file = out4b_long_pattern, which = "combined", device = img_ext, width = dev_width, height = dev_height)
src4b_long <- file.path(output_dir, sprintf("ramsey_paths_halfk_long-4.%s", img_ext))
dest4b_long <- file.path(output_dir, sprintf("fig_ramsey_paths_halfk_long.%s", img_ext))
tmp4b_long <- setdiff(Sys.glob(file.path(output_dir, sprintf("ramsey_paths_halfk_long-*.%s", img_ext))), src4b_long)
...
produce_paths_figure(
  ...
  file.path(output_dir, sprintf("fig_ramsey_paths_halfk_long_color.%s", img_ext))
)
